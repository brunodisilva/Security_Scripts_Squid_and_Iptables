...
foreach $cache (@ARGV) {
	...
	if ($cat eq 'CL' && $cs eq 'BYTES') {
		...
		$ALL{$CK}{CACHEDBYTES} += $val if ($tag eq 'UDP_HIT_OBJ');
	}
	if ($cs eq 'COUNT') {
...
print "Hitratio (docs):";
foreach $CK (@CKlist) {
	$N = $ALL{$CK}{TCPHIT} + $TAGM{$CK}{UDP_HIT_OBJ};
	$D = $ALL{$CK}{ALLTCP} + $TAGM{$CK}{UDP_HIT_OBJ};
	if ($D) {
		printf " %6d%%", 100 * $N / $D + 0.5;
	} else {
		printf " %7s", '-';
	}
}
...
print "MEAN Obj Size  :";
foreach $CK (@CKlist) {
	printf " %7d", $ALL{$CK}{BYTES} / $ALL{$CK}{ALLTCP};
}
print "\n";
